...
class OutlierEngine:
    ...
    def get_numeric_columns(self) -> List[str]:
        ...
        # Exclude ID columns (detected in validation)
        ...
        # Also exclude columns that are likely IDs (unique values == total rows)
        for col in all_numeric:
    ...
    def get_business_recommendation(self, column: str, 
        ...
        if min_val < 0 and column.lower() in ['age', 'salary', 'price', 'revenue']:
            return {
                "action": "Investigate",
        ...
        if outlier_count > 0:
            if outlier_percentage < 5:
                return {
                    "action": "Keep",
                    "reason": f"Only {outlier_percentage:.1f}% outliers in '{column}'. These may represent legitimate extreme cases."
                }
            elif outlier_percentage < 15:
                return {
                    "action": "Review",
    ...
    def generate_rankings(self) -> Dict[str, Any]:
        ...
        sorted_columns = sorted(
            self.outlier_results.items(),
            key=lambda x: x[1]["outlier_analysis"]["outlier_percentage"],
            reverse=True
        )
        
        ranking = [col for col, _ in sorted_columns if col in self.numeric_columns]
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        total_values = total_rows * len(self.numeric_columns)
        outlier_percentage_overall = (total_outliers / total_values * 100) if total_values > 0 else 0
        ...
        return {
            ...
            "riskiest_columns": [(col, result["risk_score"]) for col, result in riskiest]
        }
