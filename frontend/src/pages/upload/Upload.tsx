...
const UploadPage: React.FC = () => {
  ...
  const simulatePipeline = async () => {
    ...
    for (let i = 0; i < stageNames.length; i++) {
      const stageIndex = i + 1 // +1 because upload is index 0
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Update pipeline - upload complete
      setPipelineStages(prev => prev.map((s, idx) => 
        idx === 0 ? { ...s, status: 'completed' as const } : s
      ))
      
      // Store dataset preview info
      setDatasetPreview({
      ...
      // Run pipeline simulation (or real pipeline if available)
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'running': return 'text-primary-500 animate-pulse'
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      ...
      {/* Dataset Preview (when available) */}
      {datasetPreview && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
