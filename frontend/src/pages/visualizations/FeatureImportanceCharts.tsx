    ...
    return (
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-sm font-semibold" style={{ color: colors.text }}>Feature Importance</h3>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-5">
        ...
        <h3 className="text-sm font-semibold" style={{ color: colors.text }}>Feature Importance</h3>
        <span className="text-xs font-mono" style={{ color: colors.textMuted }}>· {featureRanking.length} features</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          >
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={chartData} layout="vertical">
                ...
                <XAxis type="number" domain={[0, 'dataMax + 0.1']} stroke={chartColors.text} tick={{ fill: chartColors.text }} />
                <YAxis dataKey="name" type="category" stroke={chartColors.text} tick={{ fill: chartColors.text }} width={80} />
                <Tooltip
                  contentStyle={{
                ...
                />
                <Bar dataKey="importance" fill={colors.accent.azure} radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          >
            ...
            <p className="text-base font-bold mt-0.5" style={{ color: colors.text }}>{chartData[0]?.name || 'N/A'}</p>
            <p className="text-xs font-mono" style={{ color: colors.accent.amber }}>
              {(chartData[0]?.percentage || 0).toFixed(1)}% impact
            </p>
          </div>
          <div 
            className="p-4 rounded-md border"
          ...
          >
            ...
            <p className="text-xs font-mono mt-0.5" style={{ color: colors.text }}>
          ...
          >
            ...
            <p className="text-base font-bold mt-0.5" style={{ color: colors.text }}>{chartData.length}</p>
          </div>
        </div>
      </div>
    </div>
